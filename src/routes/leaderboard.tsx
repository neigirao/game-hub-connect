  ...
  head: () => ({
    meta: [
      { title: "Ranking Global — Crash Coaster" },
      {
        name: "description",
        content:
          "Top 50 corridas mais épicas do Crash Coaster. Veja score, velocidade máxima, G-force e as pistas usadas por cada jogador.",
      },
    ],
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          itemListElement: [
            { "@type": "ListItem", position: 1, name: "Início", item: "https://crashcoaster.app/" },
            {
              ...
              position: 2,
              name: "Ranking Global",
              item: "https://crashcoaster.app/leaderboard",
            },
          ],
        }),
      },
...
function RankBadge({ rank }: { rank: number }) {
  if (rank <= 3) {
    return <span style={{ fontSize: 22, lineHeight: 1, flexShrink: 0 }}>{MEDAL[rank - 1]}</span>;
  }
  return (
    <span
      style={{
...
function LeaderboardRow({ row, isCurrentUser }: { row: LeaderboardRow; isCurrentUser: boolean }) {
  const stars =
    row.total_score > 75
      ? "★★★"
      : row.total_score > 55
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          {isCurrentUser && <span style={{ fontSize: 11, opacity: 0.7 }}>(você)</span>}
        </div>
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          {row.creativity_score != null && (
            <span style={{ color: "#FF6BD6" }}>🎨{row.creativity_score}</span>
          )}
        ...
        <div style={S.card}>
          {error ? (
            ...
            <div style={{ textAlign: "center", padding: "48px 0", color: "#B7AEE0" }}>
              ...
              <div style={{ fontSize: 14, marginTop: 8 }}>Seja o primeiro a entrar no ranking.</div>
              <a
                href="/play.html"
                style={{
              ...
              {rows.map((row) => (
                <div
                  ...
                  style={{
                    ...
                    animationDelay: `${Math.min(row.rank * 0.03, 0.4)}s`,
                  }}
                >
                  <LeaderboardRow row={row} isCurrentUser={row.user_id === currentUserId} />
                </div>
              ))}
            </div>
          )}

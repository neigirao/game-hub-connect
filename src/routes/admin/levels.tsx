...
function MiniTrack({ nodes, loop }: { nodes: Array<{ x: number; y: number; kind: string }>; loop: boolean }) {
  if (!nodes || nodes.length < 2) return (
    <div style={{ width: 160, height: 60, borderRadius: 8, background: "rgba(0,0,0,.25)", border: "2px dashed rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center", color: "#B7AEE0", fontSize: 11 }}>
      Sem pista
    </div>
  );

  const PAD = 8, W = 160, H = 60;
  ...
  const col = (kind: string) =>
    kind === "booster" ? "#FFA502" : kind === "brake" ? "#FF4757" : kind === "launcher" ? "#2ED573" : kind === "loop" ? "#FF6BD6" : "rgba(255,255,255,0.3)";

  return (
    <svg width={W} height={H} style={{ borderRadius: 8, background: "rgba(0,0,0,.2)" }}>
      <polyline
        points={nodes.map((n) => `${sx(n.x)},${sy(n.y)}`).join(" ")}
        fill="none" stroke="#FFE9A8" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"
      />
      {loop && nodes.length > 0 && (
        <line
      ...
      {nodes.map((n, i) => n.kind !== "normal"
        ? <circle key={i} cx={sx(n.x)} cy={sy(n.y)} r={3} fill={col(n.kind)} stroke="#0E0726" strokeWidth={1} />
        : null
      )}
    </svg>
  );
...
function LevelRow({ level, onEdit, onTogglePublish, onDelete }: {
  ...
  return (
    <div style={{
      ...
      background: "rgba(0,0,0,.25)", border: `2px solid ${level.is_published ? "#2ED573" : "#4a2aa6"}`,
      borderRadius: 14, padding: "14px 16px",
    }}>
      <div style={{ flexShrink: 0, width: 28, textAlign: "center", fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: 13, color: "#B7AEE0" }}>
        #{level.order_index}
      </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: "'Fredoka',system-ui,sans-serif", fontWeight: 700, fontSize: 16, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
          {level.title}
        </div>
        <div style={{ fontSize: 11, color: "#B7AEE0", marginTop: 2, display: "flex", gap: 10, flexWrap: "wrap" as const }}>
          ...
          <span>⭐{level.star1_score} ⭐⭐{level.star2_score} ⭐⭐⭐{level.star3_score}</span>
          <span>🪙+{level.reward_coins} ✨+{level.reward_xp}</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <span style={{
          fontFamily: "'Fredoka',system-ui,sans-serif", fontWeight: 700, fontSize: 11,
        ...
        <button onClick={() => onDelete(level)} style={{ ...S.btn("rgba(255,71,87,.2)"), color: "#FF4757", border: "1px solid rgba(255,71,87,.3)" }}>✕</button>
      </div>
    </div>
  );
}
...
const DEFAULT_FORM = {
  ...
  reward_coins: 100,
  reward_xp: 50,
  objectivesRaw: '[{"type":"survive","label":"Complete sem crashar"},{"type":"adrenaline","label":"Alcance 4G de força"}]',
  starter_track_raw: "",
  is_published: false,
};
...
function LevelForm({ initial, editingId, onSave, onCancel }: {
  initial: FormState;
  ...
  return (
    <div style={{ ...S.card, display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {error && (
        <div style={{ background: "rgba(255,71,87,.15)", border: "2px solid rgba(255,71,87,.4)", borderRadius: 10, padding: "10px 14px", color: "#FF4757", fontSize: 13 }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ gridColumn: "1 / -1" }}>
          ...
          <input style={inputStyle} value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="ex: Primeira Descida" />
        </div>
        ...
        <div>
          ...
          <select style={{ ...S.select, width: "100%" }} value={form.scenario} onChange={(e) => set("scenario", e.target.value)}>
        ...
        <div>
          ...
          <input style={inputStyle} type="number" value={form.order_index} onChange={(e) => set("order_index", Number(e.target.value))} />
        </div>

        <div>
          <label style={S.label}>Score ⭐ (1 estrela)</label>
          <input style={inputStyle} type="number" value={form.star1_score} onChange={(e) => set("star1_score", Number(e.target.value))} />
        </div>
        <div>
          <label style={S.label}>Score ⭐⭐ (2 estrelas)</label>
          <input style={inputStyle} type="number" value={form.star2_score} onChange={(e) => set("star2_score", Number(e.target.value))} />
        </div>
        <div>
          <label style={S.label}>Score ⭐⭐⭐ (3 estrelas)</label>
          <input style={inputStyle} type="number" value={form.star3_score} onChange={(e) => set("star3_score", Number(e.target.value))} />
        </div>
        <div>
          <label style={S.label}>Recompensa 🪙 Coins</label>
          <input style={inputStyle} type="number" value={form.reward_coins} onChange={(e) => set("reward_coins", Number(e.target.value))} />
        </div>
        <div>
          <label style={S.label}>Recompensa ✨ XP</label>
          <input style={inputStyle} type="number" value={form.reward_xp} onChange={(e) => set("reward_xp", Number(e.target.value))} />
        </div>

        <div style={{ gridColumn: "1 / -1" }}>
          <label style={S.label}>Objetivos (JSON)</label>
          <textarea
            style={{ ...inputStyle, minHeight: 96, resize: "vertical" as const, fontFamily: "'JetBrains Mono',monospace", fontSize: 12 }}
        ...
        <div style={{ gridColumn: "1 / -1" }}>
          ...
          <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
            <textarea
              style={{ ...inputStyle, flex: 1, minHeight: 120, resize: "vertical" as const, fontFamily: "'JetBrains Mono',monospace", fontSize: 11 }}
            ...
            <div style={{ flexShrink: 0 }}>
              ...
              {trackPreview
                ...
                : <div style={{ width: 160, height: 60, borderRadius: 8, background: "rgba(0,0,0,.25)", border: "2px dashed rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center", color: "#B7AEE0", fontSize: 11 }}>JSON inválido</div>
              }
            </div>
          </div>
          <div style={{ fontSize: 11, color: "#B7AEE0", marginTop: 4 }}>
            Dica: no <code style={{ background: "rgba(0,0,0,.3)", padding: "1px 5px", borderRadius: 4 }}>play.html</code>, salve uma pista e copie o JSON de <code style={{ background: "rgba(0,0,0,.3)", padding: "1px 5px", borderRadius: 4 }}>track_data</code> do banco.
          </div>
        </div>
        ...
        <div style={{ gridColumn: "1 / -1", display: "flex", alignItems: "center", gap: 10 }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
            ...
            <span style={{ fontFamily: "'Fredoka',system-ui,sans-serif", fontWeight: 600, fontSize: 14 }}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button
          ...
          style={{ ...S.btn("linear-gradient(180deg,#FFA502,#c97a00)"), opacity: (saving || !form.title.trim()) ? 0.5 : 1 }}
        >
...
export function AdminLevelsPage() {
  ...
  function parseObjectives(raw: string): Objective[] {
    try { return JSON.parse(raw) as Objective[]; } catch { return []; }
  }

  function parseTrack(raw: string): StarterTrack | null {
    try { const p = JSON.parse(raw) as StarterTrack; return p?.nodes ? p : null; } catch { return null; }
  }
    ...
    <div style={S.content}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.ok ? "linear-gradient(135deg,#2ED573,#1a8a46)" : "linear-gradient(135deg,#FF4757,#a02030)",
      ...
      {showForm && (
        <LevelForm
          ...
          onSave={handleSave}
          onCancel={() => { setShowForm(false); setEditingLevel(null); }}
        />
      )}
      ...
      {loading ? (
        <div style={{ ...S.card, textAlign: "center", padding: 48 }}>
          <div style={{ fontFamily: "'Fredoka',system-ui,sans-serif", fontSize: 18, color: "#B7AEE0" }}>Carregando fases...</div>
        </div>
      ) : levels.length === 0 ? (
        <div style={{ ...S.card, textAlign: "center", padding: 48 }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>🚧</div>
          <div style={{ fontFamily: "'Fredoka',system-ui,sans-serif", fontWeight: 700, fontSize: 18 }}>Nenhuma fase ainda</div>
          <div style={{ color: "#B7AEE0", marginTop: 8 }}>Clique em "Nova Fase" para criar a primeira.</div>
        </div>
      ) : (
        <>
          {published.length > 0 && (
            <div style={S.card}>
              <div style={{ fontFamily: "'Fredoka',system-ui,sans-serif", fontWeight: 700, fontSize: 14, letterSpacing: ".6px", textTransform: "uppercase", color: "#2ED573", marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#2ED573", boxShadow: "0 0 0 3px rgba(46,213,115,.25)" }} />
          ...
          {drafts.length > 0 && (
            <div style={S.card}>
              <div style={{ fontFamily: "'Fredoka',system-ui,sans-serif", fontWeight: 700, fontSize: 14, letterSpacing: ".6px", textTransform: "uppercase", color: "#B7AEE0", marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#B7AEE0", boxShadow: "0 0 0 3px rgba(183,174,224,.2)" }} />
